...
static float get_major_noise(int tick) {
	if (tick < START_GLITCH_DURATON) {
		return (START_GLITCH_DURATON - tick) * (1.f / START_GLITCH_DURATON);
	}
...
static void draw_bg(int tick, uint16_t width, uint16_t height, uint8_t* frame) {

	if (tick % 4 < 2 && tick_remainder(tick, BG_GLITCH_PERIOD) < BG_GLITCH_DURATION) {
		uint16_t min_h = height / 20;
		uint16_t max_h = height / 5;
...
static void draw_logo(int tick, uint16_t width, uint16_t height, uint8_t* frame) {

	const int32_t img_w = i32min(width, (int32_t) png_get_image_width(png_ptr, info_ptr));
	const int32_t img_h = i32min(height, (int32_t) png_get_image_height(png_ptr, info_ptr));
	const int32_t start_x = (width - img_w) / 2;
	const int32_t start_y = (height - img_h) / 2;
	const int32_t end_x = start_x + img_w;
	const int32_t end_y = start_y + img_h;

	const float major_noise = get_major_noise(tick);
	const float minor_noise = get_minor_noise(tick);

	const int32_t max_line_h    = img_h / 4;
	const int32_t max_off_x     = (int32_t) (img_w * major_noise * 0.2f);
	const int32_t min_off_color = (int32_t) (img_w * major_noise * 0.05f);
	const int32_t max_off_color = (int32_t) (img_w * major_noise * 0.1f);

	color_t **img_rows = (color_t**) png_get_rows(png_ptr, info_ptr);

	for (int32_t sy = start_y; sy < end_y; ) {
		...
		const int32_t ey = sy + i32min(end_y - sy, randrange(4, max_line_h));
		const int32_t sx = i32max(0, start_x - max_off_x);
		const int32_t ex = i32min(width, end_x + max_off_x);

		const int32_t minor_y = randrange(sy + MINOR_GLITCH_AMPLITUDE, ey - MINOR_GLITCH_AMPLITUDE);
		const int32_t amplitude = (int32_t) (MINOR_GLITCH_AMPLITUDE * minor_noise);
		const int sign = chance(MINOR_GLITCH_CHANCE) ? randchoose(-1, 1) : 0;


		for (int32_t y = sy; y < ey; y++) {
			for (int32_t x = sx; x <= ex; x++) {
				int32_t tx = x + off_x + i32max(0, amplitude - abs((int32_t)(y - minor_y))) * sign;
				...
				if (tx >= 0 && tx < width) {
					...
					int32_t index = (y * width + tx) * sizeof(color_t);
...
static void draw_system_name(int tick, uint16_t width, uint16_t height, color_t* frame) {
	const int32_t img_height = (int32_t) png_get_image_height(png_ptr, info_ptr);
	...
	for (uint32_t i = 0; i < len; i++) {
		const uint8_t ch = text_buf[i];
		if (ch < CHAR_START || ch >= CHAR_END) continue;
		
		str_width += render_glyph(ch, face)->advance_x;
	}

	int32_t sx = (width - str_width) / 2;
	int32_t baseline = (height + img_height) / 2 + TEXT_PADDING + GLYPH_HEIGHT;

	const int32_t minor_y = randrange(0, GLYPH_HEIGHT);
	const int32_t amplitude = (int32_t) (MINOR_GLITCH_AMPLITUDE * get_minor_noise(tick));
	const int sign = chance(MINOR_GLITCH_CHANCE) ? randchoose(-1, 1) : 0;
