...
static fb_info* create_fb(int dev_file, uint32_t width, uint32_t height) {
...
static void release_fb(int dev_file, fb_info* fb) {
...
void init_drm(const char* card_path) {
	dev_file = open(card_path, O_RDWR | O_CLOEXEC);
	if (dev_file < 0) {
	...
	fb_info1 = create_fb(dev_file, mode->hdisplay, mode->vdisplay);
	fb_info2 = create_fb(dev_file, mode->hdisplay, mode->vdisplay);
}
